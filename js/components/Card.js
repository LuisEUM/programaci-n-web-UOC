class Card {
  static createBasicCard(data, options = {}) {
    ...
    card.className = `card ${options.isSelected ? "selected" : ""} ${
    ...
    if (options.selectable) {
      card.appendChild(
        this.createCheckbox(options.isSelected, options.onSelect)
      );
    }
    ...
    if (options.inCollection) {
      ...
      moveButton.innerHTML =
        '<i class="fas fa-exchange-alt"></i> Mover a otra colección';
      ...
      cloneButton.innerHTML =
        '<i class="fas fa-copy"></i> Clonar a otra colección';
      ...
      removeButton.className = "remove-collection-btn";
      removeButton.innerHTML =
        '<i class="fas fa-trash-alt"></i> Remover de Colección';
  ...
  static createMetadata(data) {
    ...
    if (data.issueNumber !== undefined)
      metadataItems.push(`Issue #${data.issueNumber}`);
    if (data.price !== undefined && data.price !== null)
      metadataItems.push(`$${Number(data.price).toFixed(2)}`);
    if (data.pageCount) metadataItems.push(`${data.pageCount} páginas`);

    metadata.innerHTML = metadataItems
  ...
  static appendLists(container, data) {
    ...
    lists.forEach(({ key, label }) => {
      if (Array.isArray(data[key]) && data[key].length > 0) {
        ...
        element.innerHTML = `<strong>${label}:</strong> ${data[key]
          .slice(0, 3)
          .join(", ")}${data[key].length > 3 ? "..." : ""}`;
