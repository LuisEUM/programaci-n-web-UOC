class Tests {
    ...
    static testComic() {
        ...
        try {
            ...
            console.assert(typeof comic.id === 'number', '❌ Comic ID encapsulation failed');
    ...
    static testHero() {
        ...
        try {
            ...
            // Verificar encapsulación
            console.assert(Array.isArray(hero.comics), '❌ Hero comics encapsulation failed');
    ...
    static testFavorites() {
        ...
        try {
            ...
            console.assert(favorites.showFavorites().length > 0, '❌ Add favorite failed');
    ...
    static testReduceFunction() {
        ...
        try {
            const favorites = new Favorites();
            const comic1 = new Comic(1, "Test Comic 1", 1, "Description", 32, 
                { path: "test", extension: "jpg" }, 10, [], []);
            const comic2 = new Comic(2, "Test Comic 2", 2, "Description", 32, 
                { path: "test", extension: "jpg" }, 20, [], []);
    ...
    static testRestOperator() {
        ...
        try {
            const favorites = new Favorites();
            const comics = [
                new Comic(1, "Test Comic 1", 1, "Description", 32, 
                    { path: "test", extension: "jpg" }, 9.99, [], []),
                new Comic(2, "Test Comic 2", 2, "Description", 32, 
                    { path: "test", extension: "jpg" }, 19.99, [], [])
            ];
            
            favorites.addMultipleFavorites(...comics);
            console.assert(favorites.showFavorites().length === 2, '❌ Rest operator test failed');
    ...
    static testMapAndFilter() {
        ...
        try {
            const favorites = new Favorites();
            const comic1 = new Comic(1, "Cheap Comic", 1, "Description", 32, 
                { path: "test", extension: "jpg" }, 5, [], []);
            const comic2 = new Comic(2, "Expensive Comic", 2, "Description", 32, 
                { path: "test", extension: "jpg" }, 15, [], []);
            
            favorites.addFavoriteComic(comic1);
            favorites.addFavoriteComic(comic2);
            
            const affordableComics = favorites.getAffordableComicTitles(10);
            console.assert(Array.isArray(affordableComics), '❌ Result is not an array');
            console.assert(affordableComics.length === 1, '❌ Affordable comics count mismatch');
            console.assert(affordableComics[0] === "Cheap Comic", '❌ Affordable comic title mismatch');
    ...
    static testComicEncapsulation() {
        ...
        try {
            ...
            console.assert(comic['#id'] === undefined, '❌ ID no está correctamente encapsulado');
            console.assert(comic['#title'] === undefined, '❌ Title no está correctamente encapsulado');
            
            // Verificar getters
            console.assert(typeof comic.id === 'number', '❌ Getter de ID no funciona');
            console.assert(typeof comic.title === 'string', '❌ Getter de title no funciona');
    ...
    static testHeroEncapsulation() {
        ...
        try {
            const hero = new Hero(1, "Test Hero", "Description", "2024-01-01", 
                { path: "test", extension: "jpg" }, "test/uri", []);
            
            // Verificar que las propiedades son privadas
            console.assert(hero['#id'] === undefined, '❌ ID no está correctamente encapsulado');
            console.assert(hero['#name'] === undefined, '❌ Name no está correctamente encapsulado');
            
            // Verificar getters
            console.assert(typeof hero.id === 'number', '❌ Getter de ID no funciona');
            console.assert(typeof hero.name === 'string', '❌ Getter de name no funciona');
    ...
    static testFavoritesCollectionManagement() {
        ...
        try {
            ...
            // Test de addFavorite
            favorites.addFavoriteComic(comic1);
            console.assert(favorites.showFavorites().length === 1, '❌ addFavorite no funciona correctamente');

            // Test de addMultipleFavorites con rest operator
            favorites.addMultipleFavorites(comic1, comic2);
            console.assert(favorites.showFavorites().length === 2, '❌ addMultipleFavorites no funciona correctamente');

            // Test de copyFavorites con spread operator
            const copy = favorites.copyFavorites();
            console.assert(Array.isArray(copy), '❌ copyFavorites no retorna un array');
            console.assert(copy.length === favorites.showFavorites().length, '❌ La copia no tiene la misma longitud');

            // Test de búsqueda recursiva
            const found = favorites.findComicById(1);
            console.assert(found !== null, '❌ findComicById no encuentra el cómic');
            console.assert(found.id === 1, '❌ findComicById encuentra el cómic incorrecto');

            // Test de cálculo de precio promedio
            const average = favorites.calculateAveragePrice();
            console.assert(Math.abs(average - 14.99) < 0.01, '❌ calculateAveragePrice calcula incorrectamente');

            // Test de filtrado por precio
            const affordable = favorites.getAffordableComicTitles(15);
            console.assert(affordable.length === 1, '❌ getAffordableComicTitles filtra incorrectamente');
            console.assert(affordable[0] === "Test Comic 1", '❌ getAffordableComicTitles retorna títulos incorrectos');
