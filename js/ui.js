      ...
      try {
          ...
          for (const [collectionName, favoriteIds] of Object.entries(collections)) {
              if (favoriteIds.length > 0) {
                  ...
                  if (dataSource === 'mock') {
                      // Para datos mock, filtrar desde mockComics
                      const favorites = mockComics.comics.filter(comic => favoriteIds.includes(comic.id));
                      favorites.forEach(comic => {
                          const comicElement = this.createComicElement(comic);
                          container.appendChild(comicElement);
                      });
                  } else {
                      // Para API, obtener cada cómic individualmente
                      for (const comicId of favoriteIds) {
                          try {
                              const comic = await MarvelAPI.getComicById(comicId);
                              if (comic) {
                                  const comicElement = this.createComicElement(comic);
                                  container.appendChild(comicElement);
                              }
